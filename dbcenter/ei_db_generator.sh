...
if [[ $db_engine = "postgres" ]]; then
    # DB Engine : Postgres
    echo -e "${green_font}Postgres DB Engine Selected! Running WSO2-EI $wso2_product_version DB Scripts for Postgres...${no_color_font}"
    ...
    # DB Engine : MySQL
    echo -e "${green_font}MySQL DB Engine Selected! Running WSO2-EI $wso2_product_version DB Scripts for MySQL...${no_color_font}"
    if [[ $db_engine_version = "5.7" ]]; then
        ...
        mysql -u $cf_db_username -p$cf_db_password -h $cf_db_host -P $cf_db_port < "$db_center_path/ei/ei_db_create_mysql5.7.sql"
				...
				mysql -u $cf_db_username -p$cf_db_password -h $cf_db_host -P $cf_db_port < "$db_center_path/dbscripts/mysql5.7.sql"
				sed -i '1s/.*/USE WSO2EI_INTEGRATOR_GOV_DB;/' "$db_center_path/dbscripts/identity/mysql-5.7.sql"
				mysql -u $cf_db_username -p$cf_db_password -h $cf_db_host -P $cf_db_port < "$db_center_path/dbscripts/identity/mysql-5.7.sql"
    else
				# Crete databases
		...
		# DB Engine : Oracle
		echo -e "${green_font}Oracle DB Engine Selected! Running WSO2-EI $wso2_product_version DB Scripts for Oracle...${no_color_font}"
    ...
    # DB Engine : SQLServer
    echo -e "${green_font}SQL Server DB Engine Selected! Running WSO2-EI $wso2_product_version DB Scripts for SQL Server...${no_color_font}"
		# Crete databases
